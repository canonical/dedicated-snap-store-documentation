# Contribute to the Dedicated Snap Store documentation

This guide describes how to make a change to the Dedicated Snap Store
documentation in the `canonical/brand-store-documentation` repository.
...
```bash
git clone git@github.com:<username>/brand-store-documentation
cd brand-store-documentation
git remote add upstream git@github.com:canonical/brand-store-documentation
git fetch upstream
```
...
```bash
git clone https://github.com/<username>/brand-store-documentation
cd brand-store-documentation
git remote add upstream https://github.com/canonical/brand-store-documentation
git fetch upstream
```
...
files combine to form the document architecture. The PDF guide uses a
separate `docs/pdf-index.rst` file, so add new pages there too if they
must appear in generated PDFs.

This repository also builds customer-specific Dedicated Snap Store
documentation. Customer values live in `docs/templates/*.yaml`, and the
source files can use those values as Jinja variables. For changes to
customer templates, Read the Docs projects, PDFs, or template variables,
see `WORKFLOW.rst` before you start.
...
If your change affects customer-specific output, also build the relevant
template locally. For example:

```bash
make pdf TEMPLATE="templates/acme-alpha.yaml"
```

Use the specific template file for the customer or test case you're
updating.
